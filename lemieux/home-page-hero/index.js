(function() {
  ...
  }, y = (s, e, t, i, n = 1e3, g = 0.5) => {
    let m, c;
    if (m = new IntersectionObserver(
      function(h) {
        h[0].isIntersecting === !0 ? c = setTimeout(() => {
          w(
            e,
        ...
        }, n) : (l("Element is not fully visible", "warn"), clearTimeout(c));
      },
      { threshold: [g] }
    ), typeof s == "string") {
...
`, u = "https://ab.conversionrate.store/lemieux/home-page-hero/img", x = [
    {
      id: "new-arrivals",
      href: "/new-in/aw25",
      title: { sub: "New Arrivals", main: "AW25", collection: "Collection" },
      images: {
        desktop: `${u}/hero_new_arrivals_desktop.webp`,
        mob: `${u}/hero_new_arrivals_desktop.webp`
      }
    },
    {
      id: "clothing",
  ...
  const B = `icms-component:has(.crs-hero) > div:not(.crs-hero) {
  display: none;
}
...
>`
    ...
    init() {
      try {
        ...
        d(e, "Error initializing Hero section");
      }
    }
    async addNewHeroSection() {
      try {
        const e = await p(o.TARGET_SELECTOR);
        if (!e) {
          console.warn(`Target ${o.TARGET_SELECTOR} not found`);
          return;
        }
        (() => {
        ...
        d(e, "Error changing hero image section");
      }
    }
    replaceHeroContent(e) {
      const t = e.querySelector(o.HERO_CLASS);
      console.log("Existing hero section:", t), t && t.remove(), this.eventsAborter && this.eventsAborter.abort(), e.insertAdjacentHTML(
        "beforebegin",
        O(this.renderNewHeroSection())
      );
    }
    renderNewHeroSection() {
      const e = x.find((i) => i.id === "new-arrivals"), t = x.filter((i) => i.id !== "new-arrivals");
      return (
        /* HTML */
        `
      <div class="crs-hero">
    ...
    addStyles() {
      ...
      e.id = o.STYLES_ID, e.textContent = B, document.head.appendChild(e);
    }
    destroy() {
      var e;
      this.eventsAborter && (this.eventsAborter.abort(), this.eventsAborter = null), (e = document == null ? void 0 : document.querySelector(o.HERO_CLASS)) == null || e.remove();
    }
  };
  o.STYLES_ID = "crs-hero-styles", o.VISIBILITY_THRESHOLD = 0, o.TARGET_SELECTOR = "icms-component:has(page-component-hero-image)", o.HERO_CLASS = ".crs-hero-section";
  let b = o;
  const q = `[data-crs-hide='true']:not([data-crs-no-hide]) {
  display: none !important;
}
  ...
  display: none !important;
    ...
    init() {
      try {
        ...
        d(e, "Error initializing Hide Sections");
      }
    }
    async hideSection() {
      try {
        ...
        d(e, "Error hiding sections");
      }
    }
    processInitialComponents(e) {
      e.forEach((t) => {
        try {
          t.classList.add("crs-top-section"), this.resetHideAttribute(t), this.hideComponent(t);
        } catch (i) {
          d(i, "Processing initial component");
        }
      });
    }
    hideComponent(e) {
      var i;
      const t = (i = e.querySelector("h1, h2, h3")) == null ? void 0 : i.textContent;
      z.some((n) => t == null ? void 0 : t.includes(n)) || e.dataset.crsNoHide === "true" || (e.dataset.crsHide = "true");
    }
    ...
    addStyles() {
      ...
      const e = document.createElement("style");
      e.id = "crs-hide-section-styles", e.textContent = q, document.head.appendChild(e);
    }
  };
  f.RESIZE_DEBOUNCE_MS = 500, f.OBSERVER_CLEANUP_MS = 1e3;
  let _ = f;
  const A = (
    /* HTML */
    `<icms-component
...
>`
  ), I = `[data-crs-title='Popular Categories'] .swiper-initialized {
  overflow: hidden;
  overflow-x: auto;
...
`, v = class v {
    ...
    init() {
      try {
        ...
        d(e, "Error initializing Popular Categories");
      }
    }
    async modifySection() {
      try {
        const e = await p('[data-crs-title="Popular Categories"]');
        e && (e.outerHTML = A, this.initSlider());
      } catch (e) {
        d(e, "Error modifying Popular Categories section");
      }
    }
    initSlider() {
      ...
      e.style.cursor = "grab";
      let t = !1, i = 0, n = 0, g = 0, m = 0;
      e.addEventListener(
        "mousedown",
        (c) => {
          c.preventDefault(), t = !0, i = c.pageX, n = c.pageY, g = e.scrollLeft, m = e.scrollTop;
        },
        { signal: this.eventsAborter.signal }
      ), e.addEventListener(
        "mouseleave",
        ...
        (c) => {
          ...
          const h = c.pageX, Y = c.pageY, G = (h - i) * v.SCROLL_SENSITIVITY, V = (Y - n) * v.SCROLL_SENSITIVITY;
          e.scrollLeft = g - G, e.scrollTop = m - V;
        },
        { signal: this.eventsAborter.signal }
      ), e.addEventListener(
        "dragstart",
    ...
    addStyles() {
      ...
      const e = document.createElement("style");
      e.id = "crs-popular-categories-styles", e.textContent = I, document.head.appendChild(e);
    }
  };
  v.SCROLL_SENSITIVITY = 1;
  let S = v;
  const L = (
    /* HTML */
    `
...
`
  ), P = `.crs-outfit-section .crs-mobile page-component-text {
  ...
  class H {
    ...
    init() {
      try {
        ...
        d(e, "Error initializing Outfit Builder section");
      }
    }
    async render() {
      try {
        const e = await p(".crs-winter-sale-section");
        console.log("Outfit Builder container found", e);
        const t = document.querySelector(".crs-outfit-section");
        t && (this.eventsAborter && this.eventsAborter.abort(), t.remove());
        const i = L;
        if (e)
          e.insertAdjacentHTML("afterend", i), this.setupEvents();
        else {
          l("Container .crs-winter-sale-section not found", "warn");
          return;
        }
      } catch (e) {
        d(e, "Error rendering Outfit Builder section");
      }
    }
    setupEvents() {
    ...
    addStyles() {
      if (!document.getElementById("crs-outfit-builder-styles")) {
        const e = document.createElement("style");
        e.id = "crs-outfit-builder-styles", e.textContent = P, document.head.appendChild(e);
      }
    }
  }
  const R = "", j = (
...
`
    ...
    async render() {
      try {
        ...
        if (!e) {
          l(`Container ${r.CONTAINER_SELECTOR} not found`, "warn");
          return;
        }
    ...
    insertSection(e) {
      e.insertAdjacentHTML("beforeend", j);
    }
    setupEventListeners() {
    ...
    addStyles() {
      ...
      const e = document.createElement("style");
      e.id = r.STYLES_ID, e.textContent = R, document.head.appendChild(e);
    }
    destroy() {
        ...
        <href
          ...
          ><!----><!----><a
            ...
            <!----><!----><vimeo-player
              ...
              ><div data-vimeo-initialized="true">
                <iframe
                  src="https://player.vimeo.com/video/1147988436?title=0&amp;byline=0&amp;portrait=0&amp;badge=0&amp;color=ffffff&amp;muted=1&amp;autoplay=1&amp;controls=0&amp;loop=1&amp;background=1&amp;app_id=122963"
                  width="426"
                  height="214"
                  frameborder="0"
                  allow="autoplay; fullscreen; picture-in-picture; clipboard-write; encrypted-media; web-share"
                ...
                <div _ngcontent-ng-c534717118="" class="w-12 pos-relative">
                  <text-overlay
                    ...
                    >
                      <div
                        _ngcontent-ng-c3259544976=""
                        class="w-12 center hero _text-shadow ng-star-inserted p-t-4"
                        style="color: rgb(255, 255, 255); font-size: 119px; line-height: 119px; letter-spacing: 22px; margin-left: 11px;"
                      >
                        WINTER SALE
                      </div>
                      <!----><!----><!---->
                      <div
                        _ngcontent-ng-c3259544976=""
                        class="w-12 center p1 _text-shadow ng-star-inserted p-t-8"
                        style="color: rgb(255, 255, 255); font-size: 25px; line-height: 25px; letter-spacing: 11px; margin-left: 5.5px;"
                      >
                      ...
                      <div
                        _ngcontent-ng-c3259544976=""
                        class="w-12 center p1 _text-shadow ng-star-inserted p-t-7"
                        style="color: rgb(255, 255, 255); font-size: 25px; line-height: 25px; letter-spacing: 11px; margin-left: 5.5px;"
                      >
                        UP TO 50% OFF
                      </div>
...
>`
    ...
    async render() {
      try {
        ...
        console.log("Winter Sale container found", e), this.cleanupExistingSection(), this.insertSection(e), this.setupEventListeners(), this.setupVisibilityTracking();
      } catch (e) {
        this.handleError(e, "Rendering");
      }
    }
    ...
    insertSection(e) {
      e.insertAdjacentHTML(a.INSERTION_POSITION, N);
    }
    setupEventListeners() {
      this.eventsAborter && this.eventsAborter.abort(), this.eventsAborter = new AbortController(), this.linkConfigs.forEach((e) => {
        this.setupLinksForConfig(e);
    ...
    addStyles() {
      ...
      const e = document.createElement("style");
      e.id = a.STYLES_ID, e.textContent = D, document.head.appendChild(e);
    }
    destroy() {
      this.eventsAborter && (this.eventsAborter.abort(), this.eventsAborter = null);
      const e = document.querySelector(a.SECTION_SELECTOR);
  ...
  a.STYLES_ID = "crs-winter-sale-styles", a.CONTAINER_SELECTOR = ".crs-slider-section", a.INSERTION_POSITION = "afterend", a.SECTION_SELECTOR = ".crs-winter-sale-section", a.VISIBILITY_THRESHOLD = 0;
  let T = a;
  k({ name: "Homepage Hero Image Alternative", dev: "OS" }), (function(s, e, t, i, n, g) {
    s.hj = s.hj || function() {
      (s.hj.q = s.hj.q || []).push(arguments);
    }, s._hjSettings = { hjid: 2667925, hjsv: 6 }, n = e.getElementsByTagName("head")[0], g = e.createElement("script"), g.async = !0, g.src = t + s._hjSettings.hjid + i + s._hjSettings.hjsv, n && n.appendChild(g);
  })(window, document, "https://static.hotjar.com/c/hotjar-", ".js?sv="), window.hj("event", "exp_hp_hero");
  class $ {
    constructor() {
      this.previousUrl = location.href, this.hero = new b(), this.hideSections = new _(), this.popularCategories = new S(), this.outfitBuilder = new H(), this.christmasGifts = new E(), this.winterSale = new T(), this.init();
    }
    async init() {
    ...
    initComponents() {
      (location.pathname === "/" || location.pathname === "/us/") && this.waitAutoInitData().then(() => {
        this.addStyles(), this.hero.init(), this.hideSections.init(), this.popularCategories.init(), this.outfitBuilder.init(), this.winterSale.init();
      });
    }
    waitAutoInitData() {
    ...
    destroyComponents() {
      this.hero.destroy(), this.hideSections.destroy(), this.popularCategories.destroy(), this.outfitBuilder.destroy(), this.christmasGifts.destroy(), this.winterSale.destroy();
    }
  ...
  window._crsTestInitialized || (window._crsTestInitialized = !0, new $());
})();
