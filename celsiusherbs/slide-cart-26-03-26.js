...
#kaching-cart {
  display: none !important;
    ...
    T = () =>
      ...
      `,
    D = (a, t = !1) => {
      const e = `$${(a.price / 100).toFixed(2)}`,
        n = t ? Math.ceil(a.price * 0.8) : a.discounted_price !== a.price ? a.discounted_price : null,
        s =
          n !== null
            ? `<div class="crs-item-prices">
        <span class="crs-item-price-original">${e}</span>
        <span class="crs-item-price-sale">$${(n / 100).toFixed(2)}</span>
      </div>`
            : `<span class="crs-item-price">${e}</span>`
      return (
        /* HTML */
        `
          <div class="crs-cart-item" data-key="${a.key}">
            <img class="crs-item-img" src="${a.image || ''}" alt="${a.title}" loading="lazy" />
            <div class="crs-item-info">
              <div class="crs-item-top">
                <a href="${a.url}" class="crs-item-title" target="_blank" rel="noopener">
                  ${a.title}${a.variant_title && a.variant_title !== 'Default Title' ? ` — ${a.variant_title}` : ''}
                </a>
              ...
              <div class="crs-item-bottom">
                ...
                <div class="crs-qty-stepper">
                  ...
                  <span class="crs-qty-value">${a.quantity}</span>
    ...
    E = a =>
      /* HTML */
      `
        <div class="crs-bundle-product" data-variant-id="${a.variantId}">
          <div class="crs-bundle-top">
            <img class="crs-bundle-img" src="${a.image}" alt="${a.title}" loading="lazy" />
            <a href="/products/${a.handle}" class="crs-bundle-product-title" target="_blank" rel="noopener"
              >${a.title}</a
            >
          </div>
          <div class="crs-bundle-bottom">
            <div class="crs-bundle-prices">
              <span class="crs-bundle-original-price">$${(a.originalPrice / 100).toFixed(2)}</span>
              <span class="crs-bundle-sale-price">$${(a.discountedPrice / 100).toFixed(2)}</span>
            </div>
            <button class="crs-add-to-bundle-btn" data-action="add-bundle" data-variant-id="${a.variantId}">ADD</button>
          </div>
        </div>
      `,
    O =
          ...
          <div class="crs-cart-panel">
            ...
            <div class="crs-cart-body">
              ...
              <div class="crs-bundle-section crs-hidden">
                <p class="crs-bundle-heading">
                  Complete your bundle &amp; <span class="crs-orange-text">save 20%</span>
                </p>
  ...
  class R {
    ...
    async init() {
      ;(await $('body'),
        document.head.insertAdjacentHTML('beforeend', `<style class="crs-slide-cart-style">${M}</style>`),
        document.body.insertAdjacentHTML('beforeend', O),
        (this.overlay = document.getElementById('crs-slide-cart')),
        await this.injectCartButton(),
        this.bindClose(),
        this.bindCartItemActions(),
        this.blockKachingBodyLock(),
        this.injectAddToCartOverlays(),
        (this.cart = await this.fetchCart()),
        this.updateBadge(this.cart.item_count))
    }
    blockKachingBodyLock() {
      const t = 'kaching-body__cart-open',
        e = s => s.classList.contains(t) && s.classList.remove(t),
        n = new MutationObserver(() => {
          ;(e(document.body), e(document.documentElement))
        })
      ;(n.observe(document.body, { attributes: !0, attributeFilter: ['class'] }),
    ...
    updateBadge(t) {
      const e = document.querySelector('.crs-cart-btn-badge')
      e && ((e.textContent = String(t)), e.classList.toggle('crs-hidden', t === 0))
    }
    ...
    renderCart() {
      var f, g, m, x, b, k, S
      if (!this.cart) return
      const t = (f = this.overlay) == null ? void 0 : f.querySelector('.crs-cart-items-wrapper'),
        e = (g = this.overlay) == null ? void 0 : g.querySelector('.crs-cart-title'),
        n = (m = this.overlay) == null ? void 0 : m.querySelector('.crs-subtotal-amount')
      e && (e.textContent = `Your cart (${this.cart.item_count} ${this.cart.item_count === 1 ? 'item' : 'items'})`)
      const s = this.cart.item_count >= C,
        r = this.cart.original_total_price,
        o = r > 0 ? Math.round(((r - this.cart.total_price) / r) * 100) : 0,
        i = s && o < 20,
        c = i
          ? this.cart.items.reduce((v, L) => v + Math.ceil(L.price * 0.8) * L.quantity, 0)
          : this.cart.total_price !== r
            ? this.cart.total_price
            : r,
        l = c !== r,
        d = (x = this.overlay) == null ? void 0 : x.querySelector('.crs-subtotal-original')
      ;(n && (n.textContent = `$${(c / 100).toFixed(2)}`),
        d && ((d.textContent = `$${(r / 100).toFixed(2)}`), d.classList.toggle('crs-hidden', !l)))
      const p = (b = this.overlay) == null ? void 0 : b.querySelector('.crs-discount-info'),
        u = (k = this.overlay) == null ? void 0 : k.querySelector('.crs-discount-savings')
      if ((p && p.classList.toggle('crs-hidden', !l), l && u)) {
        const v = Math.round((1 - c / r) * 100)
        u.textContent = `-${v}%`
      }
      t && (t.innerHTML = this.cart.items.map(v => D(v, i)).join(''))
      const h = (S = this.overlay) == null ? void 0 : S.querySelector('.crs-checkout-btn')
      ;(h && (h.href = s ? `/checkout?discount=${A}` : '/checkout'),
        this.updateDiscountBar(this.cart.item_count),
        this.updateBundleSection(this.cart.item_count))
    }
    updateDiscountBar(t) {
      var r, o
      const e = (r = this.overlay) == null ? void 0 : r.querySelector('.crs-progress-fill'),
        n = (o = this.overlay) == null ? void 0 : o.querySelector('.crs-discount-text'),
        s = Math.min(100, Math.round((t / C) * 100))
      if ((e && (e.style.width = `${s}%`), n))
        if (t >= C)
    ...
    async updateBundleSection(t) {
      var p, u, h, f, g, m, x
      const e = (p = this.overlay) == null ? void 0 : p.querySelector('.crs-bundle-section'),
        n = (u = this.overlay) == null ? void 0 : u.querySelector('.crs-bundle-products-list'),
        s = (h = this.overlay) == null ? void 0 : h.querySelector('.crs-bundle-heading')
      if (!e || !n) return
      if (t === 0 || t >= C) {
        e.classList.add('crs-hidden')
        return
      }
      const r = this.cart.items[0],
        o =
          (r == null ? void 0 : r.handle) ??
          ((x =
            (m =
              (g = (f = r == null ? void 0 : r.url) == null ? void 0 : f.split('/products/')) == null
                ? void 0
                : g[1]) == null
              ? void 0
              : m.split('?')) == null
      ...
      s && (s.textContent = i.cartHeader)
      const c = this.cart.items.map(b => b.variant_id),
        d = (
          await Promise.all(
            i.shows.map(async b => {
              var k, S, v
              try {
                const L = await fetch(`/products/${b}.js`)
                if (!L.ok) return null
                const H = await L.json(),
                  _ = (k = H.variants) == null ? void 0 : k[0]
                ...
                return {
                  ...
                  title: H.title,
                  image:
                    H.featured_image ?? ((v = (S = H.images) == null ? void 0 : S[0]) == null ? void 0 : v.src) ?? '',
                  handle: b,
                  originalPrice: q,
      ...
      ;((n.innerHTML = d.map(E).join('')), e.classList.remove('crs-hidden'))
    }
    bindCartItemActions() {
      var t
      ;(t = this.overlay) == null ||
        t.addEventListener('click', e => {
          ...
          if (s === 'add-bundle') {
            const g = n.dataset.variantId
            if (g) {
              const m =
                ((u =
                  (p =
                    (d = n.closest('.crs-bundle-product')) == null
                      ? void 0
                      : d.querySelector('.crs-bundle-product-title')) == null
                    ? void 0
                    : p.textContent) == null
                  ? void 0
                  : u.trim()) || g
              ;(y('slide_cart_bundle_add', `Bundle add: ${m}`, 'click', 'Cart Bundle'), this.addToCart(Number(g)))
            }
            return
          }
          ...
          const o = r.dataset.key,
            i = r.querySelector('.crs-qty-value'),
            c = parseInt((i == null ? void 0 : i.textContent) || '1', 10),
            l =
              ((f = (h = r.querySelector('.crs-item-title')) == null ? void 0 : h.textContent) == null
                ? void 0
                : f.trim()) || o
          s === 'remove'
            ? (y('slide_cart_item_remove', `Remove: ${l}`, 'click', 'Cart'), this.updateCart(o, 0))
            : s === 'decrease'
              ? (y('slide_cart_item_decrease', `Qty decrease: ${l}`, 'click', 'Cart'),
                this.updateCart(o, Math.max(0, c - 1)))
              : s === 'increase' &&
                (y('slide_cart_item_increase', `Qty increase: ${l}`, 'click', 'Cart'), this.updateCart(o, c + 1))
        })
    }
    async addToCart(t, e = 1) {
      ;(
    ...
    open() {
      var t
      ;(y('slide_cart_open', 'Cart opened', 'view', 'Slide Cart'),
        this.renderCart(),
        (t = this.overlay) == null || t.classList.add('crs-is-open'))
    }
    close() {
      var t
        ...
        new Promise(n => {
          ...
          const s = setTimeout(() => {
            ...
            r = new MutationObserver(() => {
              document.querySelector(e) && (clearTimeout(s), r.disconnect(), n(!0))
            })
          r.observe(document.documentElement, { childList: !0, subtree: !0 })
        })
      ;(t('a.btn.add-to-cart').then(e => {
        e &&
          document.querySelectorAll('a.btn.add-to-cart').forEach(n => {
            const s = () => Number(n.id)
            this.attachOverlay(n, s, !1)
          })
      }),
        t('button.zpa-add-to-cart-btn').then(e => {
          e &&
            document.querySelectorAll('button.zpa-add-to-cart-btn').forEach(n => {
              const s = n.querySelector('.zpa-btn-custom__caption')
              s && (s.textContent = 'ADD TO CART')
              const r = () => {
                  var p, u, h
                  const i = n.closest('form'),
                    c = i == null ? void 0 : i.querySelector('select[name="id"], input[name="id"]')
                  if (c && Number(c.value)) return Number(c.value)
                  const l = n.closest('section') ?? ((p = n.closest('[data-id]')) == null ? void 0 : p.closest('div')),
                    d = l == null ? void 0 : l.querySelector('script[data-zp-product-init]')
                  if (d)
                    try {
                      const f = JSON.parse(d.textContent || '{}')
                      return (
                        f.selected_variant_id ??
                        ((h = (u = f.variants) == null ? void 0 : u[0]) == null ? void 0 : h.id) ??
                        0
                      )
                    } catch {
                      return 0
                ...
                o = () => {
                  const i = n.closest('section') ?? n.closest('[data-product-wrapper-id]'),
                    c =
                      (i == null ? void 0 : i.querySelector('.zpa-quantity-field')) ??
                      document.querySelector('.zpa-quantity-field')
                  return Math.max(1, parseInt((c == null ? void 0 : c.value) || '1', 10) || 1)
                }
              this.attachOverlay(n, r, !0, o)
            })
        }),
        t('button[data-pf-type="ProductATC"]').then(e => {
          if (!e) return
          const n = async o => {
              var p, u, h, f, g, m
              if (o.closest('.crs-atc-btn-wrapper')) return
              const i =
                ((p = o.closest('[data-product-id]')) == null ? void 0 : p.querySelector('a[href*="/products/"]')) ??
                ((h = (u = o.closest('form')) == null ? void 0 : u.parentElement) == null
                  ? void 0
                  : h.querySelector('a[href*="/products/"]'))
              if (!i) return
              const c = (f = i.pathname.split('/products/')[1]) == null ? void 0 : f.split('?')[0]
              if (!c) return
              const l = await fetch(`/products/${c}.js`).then(x => x.json()),
                d = (m = (g = l == null ? void 0 : l.variants) == null ? void 0 : g[0]) == null ? void 0 : m.id
    ...
    attachOverlay(t, e, n = !1, s) {
      let r
      if (n) {
        const i = document.createElement('div')
        ;((i.className = 'crs-atc-btn-wrapper'), t.parentElement.insertBefore(i, t), i.appendChild(t), (r = i))
      } else ((r = t.parentElement), getComputedStyle(r).position === 'static' && (r.style.position = 'relative'))
      const o = document.createElement('div')
      ;((o.className = 'crs-atc-overlay'),
        r.appendChild(o),
        o.addEventListener('click', async i => {
          ...
          if (!(!c || o.classList.contains('crs-atc-loading'))) {
            ...
            try {
              const p = s ? s() : 1
              await this.addToCart(c, p)
              const u =
                ((d = (l = this.cart) == null ? void 0 : l.items.find(h => h.variant_id === c)) == null
                  ? void 0
                  : d.title) || String(c)
              ;(y('slide_cart_add', `Add to cart: ${u}`, 'click', 'ATC Button'), this.open())
            } finally {
    ...
    async injectCartButton() {
      const t = await $('a.hdicon[href="/cart"], a[data-icon-destination-type="cart_dest"]')
      t.style.display = 'none'
      const e = t.hasAttribute('data-icon-destination-type')
      ;(t.insertAdjacentHTML('afterend', e ? T() : j()),
        t.nextElementSibling.addEventListener('click', () => this.open()))
    }
    bindClose() {
      var t, e, n, s, r, o
      ;((e = (t = this.overlay) == null ? void 0 : t.querySelector('.crs-cart-close')) == null ||
        e.addEventListener('click', () => this.close()),
        (s = (n = this.overlay) == null ? void 0 : n.querySelector('.crs-cart-backdrop')) == null ||
          s.addEventListener('click', () => this.close()),
        (o = (r = this.overlay) == null ? void 0 : r.querySelector('.crs-checkout-btn')) == null ||
          o.addEventListener('click', () => {
            y('slide_cart_checkout', 'Checkout clicked', 'click', 'Slide Cart')
          }))
    }
  }
  new R()
