...
`,l=`.auth-social-buttons`,u={google:`.auth-google-identity-button-wrapper`,facebook:`.auth-facebook-login-button`,apple:`.auth-apple-sign-in-button`};function d(){o(l,t=>{r(l,`auth_social_view`,`auth_popup`,`Social auth buttons entered the viewport`),t.addEventListener(`click`,t=>{let n=t.target,r=Object.keys(u).find(e=>n.closest(u[e]));r&&e(`auth_social_click`,`Click on ${r} button`,`click`,`auth_popup`)})})}var f={movie:`movie`,series:`series`,tv:`channel`,"free-tv":`channel`},p=e=>`https://sweet.tv/cdn-cgi/image/f=auto,q=80,fit=cover,w=534,h=300/${e}`,m=`https://sweet-tv-static.sweet.tv/web/nuxt/pages/tv/player-frame/bg.png`;function h(e=10){let t=(`useNuxtApp`in window?window.useNuxtApp():null)?.$pinia?_():null;if(t){new Image().src=t.posterUrl;return}e&&setTimeout(()=>h(e-1),300)}function g(e,t=20){let n=_();if(n){e(n);return}t&&setTimeout(()=>g(e,t-1),100)}function _(){let[,,e,t]=location.pathname.split(`/`),n=f[e];if(!n)return null;if(e===`tv`&&!t)return{kind:n,title:``,posterUrl:p(m),durationMin:null,rating:null};if(!t)return null;let r=window.useNuxtApp();if(n===`channel`){let e=r.$pinia.state.value.tvList.tvCurrentChannel;return e?{kind:n,title:e.title,posterUrl:p(e.banner_url),durationMin:null,rating:null}:null}let i=location.pathname.split(`/`).filter(Boolean).join(`:`),a=r.payload.data[`movie-info:${i}`]?.movie;if(!a)return null;let o=a.scores?.find(e=>e.provider===`IMDB`);return{kind:n,title:a.year?`${a.title} (${a.year})`:a.title,posterUrl:p(a.horizontal_poster_url||a.banner_url||a.poster_url),durationMin:a.duration||null,rating:o?.value??null}}function v(e,t){if(!(t==null||typeof t==`boolean`)){if(Array.isArray(t)){for(let n of t)v(e,n);return}e.appendChild(t instanceof Node?t:document.createTextNode(String(t)))}}function y(e,t,n){if(typeof e==`function`)return e(t??{});let{children:r,...i}=t??{},a=document.createElement(e);for(let[e,t]of Object.entries(i))e.startsWith(`on`)&&typeof t==`function`?a.addEventListener(e.slice(2).toLowerCase(),t):t===!0?a.setAttribute(e,``):t!==!1&&t!=null&&a.setAttribute(e,String(t));return v(a,r),a}var b=y,x={movie:`фільм`,series:`серіал`,channel:`канал`},S=e=>{let t=Math.floor(e/60),n=e%60;return t?`${t}h ${n}m`:`${n}m`},C=e=>b(`div`,{class:`crs-side-panel__meta`,children:[e.durationMin?y(`span`,{children:S(e.durationMin)}):``,e.durationMin&&e.rating?y(`span`,{children:`·`}):``,e.rating?y(`span`,{class:`crs-side-panel__rating`,children:String(e.rating)}):``]}),w=e=>{let t=b(`aside`,{class:`crs-side-panel`,children:[y(`div`,{class:`crs-side-panel__backdrop`,style:`background-image:url(${e.posterUrl})`}),b(`div`,{class:`crs-side-panel__body`,children:[b(`p`,{class:`crs-side-panel__caption`,children:[`Твій `,x[e.kind],` чекає`]}),y(`div`,{class:`crs-side-panel__poster`,children:y(`img`,{class:`crs-side-panel__poster-img`,src:e.posterUrl,fetchpriority:`high`,alt:``})}),e.title?y(`p`,{class:`crs-side-panel__title`,children:e.title}):``]}),y(`p`,{class:`crs-side-panel__note`,children:`Українською в HD · одразу після активації`})]});return(e.durationMin||e.rating)&&t.querySelector(`.crs-side-panel__poster`).append(C(e)),t},T=`.ui-modal__dialog:has(.crs-side-panel) {
...
}`,E=`.auth-modal-shell`;function D(){o(E,t=>{g(n=>{t.append(w(n)),`${n.kind}`,n.title&&`${n.title}`,e(`auth_panel_view`,`Panel with content: ${n.kind}`,`view`,`auth_popup`)})})}var O=`.auth-v1-start-screen__phone-input .ui-input__field {
  display: flex !important;
  align-items: stretch !important;
  gap: 8px !important;
  padding: 0 !important;
...
`,k=`.auth-v1-start-screen__phone-input .ui-input__input`;function A(){o(k,e=>{e.placeholder=`93 000 00 00`})}var j={movie:`фільму`,series:`серіалу`,channel:`каналу`},M=e=>b(`div`,{class:`crs-stepper`,children:[b(`div`,{class:`crs-stepper__step crs-stepper__step--done`,children:[y(`span`,{class:`crs-stepper__badge`,children:`1`}),b(`span`,{class:`crs-stepper__label`,children:[`Активація`,y(`br`,{}),`доступу`]})]}),y(`span`,{class:`crs-stepper__divider`}),b(`div`,{class:`crs-stepper__step`,children:[y(`span`,{class:`crs-stepper__icon`}),b(`span`,{class:`crs-stepper__label`,children:[`Перегляд`,y(`br`,{}),j[e.kind]]})]})]}),N=`.auth-modal-shell:has(.crs-side-panel) .auth-v1-start-screen__body-secure,
...
`,P=`.auth-v1-start-screen`,F={".auth-v1-start-screen__body-info-title":`Активуй безкоштовний доступ`,".auth-v1-start-screen__body-info-text":`Надішлемо SMS-код — це твій вхід без пароля`,".auth-v1-start-screen__body-options-title":`або увійди за 1 клік`},I=`Отримати код і дивитися`;function L(){o(P,e=>{g(t=>{Object.entries(F).forEach(([t,n])=>{e.querySelector(t).textContent=n}),e.querySelector(`.auth-v1-start-screen__body-form-button`).setAttribute(`aria-label`,I),e.querySelector(`.auth-v1-start-screen__body-info`).prepend(M(t));let n=document.createElement(`p`);n.className=`crs-trial-badge`,n.textContent=`7 днів безкоштовно · Без банківської картки`,e.querySelector(`.auth-v1-start-screen__body-form`).append(n),`${t.kind}`})})}var R=`
...
}`;t({name:`sweettv testing`,dev:`OS`}),n(`exp_test`);var z=`crs-auth-popup`;new class{constructor(){this.init()}init(){window.IS_AUTH!==!0&&(this.ensureStyles([``,T,N,O,R,c]),h(),!window.__crsAuthPopupInit&&(window.__crsAuthPopupInit=!0,D(),L(),A(),d()))}ensureStyles(e){queueMicrotask(()=>{if(document.getElementById(z))return;let t=document.createElement(`style`);t.id=z,t.textContent=e.join(`
