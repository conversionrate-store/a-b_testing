...
`,x=[[`.movie-content__title`,`.movie-content__actions-watch`,`.movie-content`],[`.stub__info-title`,`.tv-mobile__stub-action-button, .stub__action-button`,`.stub`]];function S(){c(()=>{if(h()){document.querySelectorAll(`.crs-play`).forEach(e=>e.remove());return}x.forEach(([t,n,r])=>{let i=document.querySelector(t);if(!i||i.querySelector(`.crs-play`))return;let a=y({onPlay:()=>{let t=[...(i.closest(r)??document).querySelectorAll(n)].find(u)??[...document.querySelectorAll(n)].find(u);if(!t){`${n}`;return}e(`exp_play_btn_click`,`Play button`,`click`,`Content Page`),t.click()}});i.prepend(a)})})}var C=`html:not(.crs-unreleased) .episode-card__image::after {
  content: '';
  position: absolute;
  inset: 0;
  margin: auto;
...
`,E={tv:`.stub__action-button, .tv-mobile__stub-action-button`,movie:`.movie-content__actions-watch`,series:`.movie-content__actions-watch`},D=`Дивитися`;function O(){c(()=>{let e=m();if(!e||!d[e]||h())return;let t=g();t&&document.querySelectorAll(E[e]).forEach(e=>{let n=[...e.childNodes].find(e=>e.nodeValue?.includes(D));if(!n)return;e.dataset.crsLabel??=n.nodeValue;let r=`${t} ${e.dataset.crsLabel.replace(D,``).trim()}`;n.nodeValue!==D&&(n.nodeValue=D);let i=e.querySelector(`.crs-cta-title`);i||(i=document.createElement(`span`),i.className=`crs-cta-title`,n.after(i)),i.textContent!==r&&(i.textContent=r)})})}t({name:`Additional play buttons`,dev:`OS`}),n(`exp_play_btns`);var k=`crs-play-btns`;new class{constructor(){this.init()}init(){this.ensureStyles([r,b,C,T]),!window.__crsPlayBtnsInit&&(window.__crsPlayBtnsInit=!0,w(),s(()=>{c(()=>{document.documentElement.classList.toggle(`crs-unreleased`,h())}),S(),O()}))}ensureStyles(e){queueMicrotask(()=>{if(!document.head){requestAnimationFrame(()=>this.ensureStyles(e));return}if(document.getElementById(k))return;let t=document.createElement(`style`);t.id=k,t.textContent=e.join(`
