...
`,h=[[`.movie-content__title`,`.movie-content__actions-watch`,`.movie-content`],[`.stub__info-title`,`.tv-mobile__stub-action-button, .stub__action-button`,`.stub`]];function g(){c(()=>{h.forEach(([t,n,r])=>{let i=document.querySelector(t);if(!i||i.querySelector(`.crs-play`))return;let a=p({onPlay:()=>{let t=[...(i.closest(r)??document).querySelectorAll(n)].find(u)??[...document.querySelectorAll(n)].find(u);if(!t){`${n}`;return}e(`exp_play_btn_click`,`Play button`,`click`,`Content Page`),t.click()}});i.prepend(a)})})}var _=`.episode-card__image::after {
  content: '';
  position: absolute;
  inset: 0;
  margin: auto;
...
`;function v(){document.addEventListener(`click`,e=>{e.target.closest(`.episode-card__image`)},!0)}var y={tv:!0,movie:!1,series:!1},b={tv:`tv`,movie:`movie`,series:`series`};function x(){return b[location.pathname.split(`/`)[2]]??null}function S(){let[,e,t,n]=location.pathname.split(`/`),r=window.useNuxtApp?.();return r?x()===`tv`?r.$pinia.state.value.tvList?.tvCurrentChannel?.title??``:r.payload.data[`movie-info:${e}:${t}:${n}`]?.movie?.title??``:``}var C=`/* Both lines keep the host's own type scale — per design — so the pill is sized by
...
`,w={tv:`.stub__action-button, .tv-mobile__stub-action-button`,movie:`.movie-content__actions-watch`,series:`.movie-content__actions-watch`},T=`Дивитися`;function E(){c(()=>{let e=x();if(!e||!y[e])return;let t=S();t&&document.querySelectorAll(w[e]).forEach(e=>{let n=[...e.childNodes].find(e=>e.nodeValue?.includes(T));if(!n)return;e.dataset.crsLabel??=n.nodeValue;let r=`${t} ${e.dataset.crsLabel.replace(T,``).trim()}`;n.nodeValue!==T&&(n.nodeValue=T);let i=e.querySelector(`.crs-cta-title`);i||(i=document.createElement(`span`),i.className=`crs-cta-title`,n.after(i)),i.textContent!==r&&(i.textContent=r)})})}t({name:`Additional play buttons`,dev:`OS`}),n(`exp_play_btns`);var D=`crs-play-btns`;new class{constructor(){this.init()}init(){this.ensureStyles([r,m,_,C]),!window.__crsPlayBtnsInit&&(window.__crsPlayBtnsInit=!0,v(),s(()=>{g(),E()}))}ensureStyles(e){queueMicrotask(()=>{if(!document.head){requestAnimationFrame(()=>this.ensureStyles(e));return}if(document.getElementById(D))return;let t=document.createElement(`style`);t.id=D,t.textContent=e.join(`
