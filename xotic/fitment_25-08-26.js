...
(function() {
	...
	var l = [
		...
		if (t?.classList.contains("cm_verify-fitment__yes") || t?.classList.contains("cm_verify-fitment__no")) {
			let n = t.classList.contains("cm_verify-fitment__yes") ? "yes" : "no", r = (e.querySelector(".cm_verify-fitment_vehicle")?.textContent || "").trim(), i = e.querySelector("a.cm_verify-fitment_button-compatible-parts"), a = Array.from(e.querySelectorAll(".cm_verify-fitment_vehicle-notes li")).map((e) => (e.textContent || "").trim()).filter((e) => e !== "");
			return {
				template: "locked",
				locked: {
					outcome: n,
...
`, x = (e, t) => {
  ...
  `;
		...
		let i = e === "yes", a = r.length ? `<ul class="xtc-fit__result-notes">${r.map((e) => `<li>${h(e)}</li>`).join("")}</ul>` : "";
		return `
    <div class="xtc-fit xtc-fit--result xtc-fit--${e}">
      ...
      <div class="xtc-fit__result-footer">
        ${i ? "" : `<a class="xtc-fit__cta" href="${g(n)}">SEE COMPATIBLE PRODUCTS</a>`}
  ...
  `;
	...
	new class {
		...
		renderFromLiveDom() {
			...
			if (n.kind === "result") {
				this.mount.innerHTML = S(n.outcome, n.vehicleLine, n.compatiblePartsHref, n.notes), this.mount.querySelector(".xtc-fit__change")?.addEventListener("click", () => {
