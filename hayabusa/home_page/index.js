...
`;function J(){return document.documentElement?Promise.resolve():new Promise(e=>{let t=()=>document.documentElement?e():setTimeout(t);t()})}n({name:`HP`,dev:`OS`}),r(`exp_hp`);var Y=`.page-transition > .flex.flex-col`,X=[`Back To The Grind`,`BACK TO THE GRIND - FEATURES`,`PRECISION FIT GIS`,`THREE ALL-NEW GIS`,`HAYABUSA WRESTLING`,``,`Boxing Shoes`,`SHOP BY COLLECTION`,``,`HAYABUSA BLOG`,``];function Z(e){X.forEach((t,n)=>{if(!t)return;let r=e.children[n]?.querySelector(`h2`)?.textContent?.trim();r!==t&&a(`Section ${n+1} drifted — expected "${t}", got "${r}"`,`error`)})}async function Q(){await J(),f(q,`exp-hp-styles`),Z(await t(Y)),P();let e=()=>document.querySelector(Y);d(K(),e),d(k(),e),d(await H(),e)}window.top===window.self&&s(Q)})();
