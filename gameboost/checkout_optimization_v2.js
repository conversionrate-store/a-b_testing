!(function () {
  ...
  class e {
    ...
    totalSummaryObserver() {
      ...
      t &&
        (r.observe(t, { childList: !0, subtree: !0 }), this.observers.push(r));
    }
    ...
    changeWarrantyTextPosition() {
      ...
      if (!e || !t) return;
      (t.dataset.crsOriginalText ||
        (t.dataset.crsOriginalText = t.textContent || ''),
        e.classList.contains('sm:hidden') &&
          e.setAttribute('data-crs-warranty-was-hidden', '1'),
        null ==
    ...
    cleanupArtifacts() {
      this.container &&
        (this.container
          .querySelectorAll('.crs-product-benefit-with-tooltip')
          .forEach((n) => {
        ...
        this.container
          .querySelectorAll('.crs-product-benefits-divider')
          .forEach((n) => n.remove()),
        this.container
          .querySelectorAll('.crs-product-benefits')
          .forEach((n) => {
            const e = n.parentElement;
            if (e) {
              for (; n.firstChild; ) e.insertBefore(n.firstChild, n);
        ...
        this.container.querySelectorAll('.crs-checkout-button').forEach((n) => {
          var e;
          (n.classList.remove('crs-checkout-button'), delete n.dataset.price);
          const t =
        ...
        this.container.querySelectorAll('.crs-left-header').forEach((n) => {
          var e;
          n.classList.remove('crs-left-header');
          const t =
            null == (e = this.container)
              ? void 0
              : e.querySelector(
                  "[aria-labelledby='section-one-heading'] > div > div",
                );
          null == t || t.prepend(n);
        }),
        this.container
          .querySelectorAll('.crs-product-container')
          .forEach((n) => {
            n.classList.remove('crs-product-container');
          }),
        this.container.querySelectorAll('.crs-right-header').forEach((n) => {
          for (var e; n.firstChild; )
            null == (e = n.parentElement) || e.insertBefore(n.firstChild, n);
          n.remove();
        }),
        this.container
          .querySelectorAll('[data-crs-original-html]')
          .forEach((n) => {
            ((n.innerHTML = n.dataset.crsOriginalHtml || ''),
              delete n.dataset.crsOriginalHtml);
          }),
        this.container
          .querySelectorAll('[data-crs-original-text]')
          .forEach((n) => {
            ((n.textContent = n.dataset.crsOriginalText || ''),
              delete n.dataset.crsOriginalText);
          }),
        this.container
          .querySelectorAll('[data-crs-warranty-was-hidden]')
          .forEach((n) => {
            (n.classList.add('sm:hidden'),
              n.removeAttribute('data-crs-warranty-was-hidden'));
          }));
    }
    changeProductLayout() {
      var n;
      ...
      if (t) {
        ...
        if (e) {
          ...
          const r = t.querySelector('.crs-product-benefits'),
            a = document.createElement('hr');
          ((a.className = 'crs-product-benefits-divider'),
            r &&
              ...
              r.append(e[0]),
              (null == e ? void 0 : e[1]) && r.append(a, e[1])),
            e.forEach((n) => {
              var e;
              ...
              ((null == i
                ...
                (i.dataset.crsOriginalHtml ||
                  (i.dataset.crsOriginalHtml = i.innerHTML),
    ...
    destroy() {
      ...
      this.destroyed ||
        ((this.destroyed = !0),
        this.observers.forEach((n) => n.disconnect()),
        (this.observers = []),
        this.listenerCleanup.forEach((n) => n()),
        (this.listenerCleanup = []),
        this.cleanupArtifacts(),
        null == (n = document.getElementById('crs-layout-changes-styles')) ||
          n.remove());
    }
  }
  class t {
    ...
    async addMoreButton() {
      ...
      ((i.className = 'pm-more-button-container'),
        ...
        (this.radioGroup = t),
        (this.buttonContainer = i),
        this.expandIfSelectedIsHidden(t, i));
      const r = i.querySelector('.pm-more-button');
      ((this.clickHandler = () => {
        (this.togglePaymentMethods(t, i),
          ...
          })('exp_payment_more', 'More payment methods', 'click', 'Checkout'));
      }),
        null == r || r.addEventListener('click', this.clickHandler));
    }
    ...
    destroy() {
      ...
      if (
        ((this.expanded = !1),
        this.radioGroup && this.radioGroup.classList.remove('pm-expanded'),
        this.buttonContainer)
      ) {
        this.buttonContainer.classList.remove('pm-expanded');
        const n = this.buttonContainer.querySelector('.pm-more-button');
        (n &&
          this.clickHandler &&
          n.removeEventListener('click', this.clickHandler),
          this.buttonContainer.remove());
      }
      ((this.radioGroup = null),
        (this.buttonContainer = null),
        (this.clickHandler = null),
        null == (n = document.getElementById('crs-payment-methods-styles')) ||
          n.remove());
    }
  }
  new (class {
    constructor() {
      ((this.layoutChanges = null),
        (this.paymentMethodsChanges = null),
        (this.pageObserver = null),
        this.init());
    }
    init() {
      (this.runChanges(),
        this.handlePageChange(() => this.runChanges()),
        this.addStyles());
    }
    runChanges() {
      var n, i;
      (null == (n = this.layoutChanges) || n.destroy(),
        null == (i = this.paymentMethodsChanges) || i.destroy(),
        (this.layoutChanges = new e()),
        (this.paymentMethodsChanges = new t()));
    }
    handlePageChange(n) {
      var e;
      let t = location.pathname;
      (null == (e = this.pageObserver) || e.disconnect(),
    ...
    addStyles() {
      if (document.getElementById('crs-global-styles')) return;
      const n = document.createElement('style');
      ((n.id = 'crs-global-styles'),
        (n.textContent = ''),
        document.head.appendChild(n));
    }
    destroy() {
      var n, e, t, i;
      (null == (n = this.pageObserver) || n.disconnect(),
        (this.pageObserver = null),
        null == (e = this.layoutChanges) || e.destroy(),
        null == (t = this.paymentMethodsChanges) || t.destroy(),
        (this.layoutChanges = null),
        (this.paymentMethodsChanges = null),
        null == (i = document.getElementById('crs-global-styles')) ||
          i.remove());
    }
  })();
})();
