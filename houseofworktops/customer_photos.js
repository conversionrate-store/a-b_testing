(function() {
  ...
  const c = (s, t, e, n = "") => {
    ...
    if (r = new IntersectionObserver(
      function(l) {
        ...
        }, o) : (u("Element is not fully visible", "warn"), clearTimeout(a));
      },
      { threshold: [i] }
    ), typeof s == "string") {
  ...
  }, E = `/* Customer Photos — mirrors the reels-section look (green #093b32, gold heading, square peek slider). */
...
.crs-cp+.container {
  ...
  function y(s) {
    ...
    t ? t(s, { timeout: 2e3 }) : setTimeout(s, 1200);
  }
  const A = "https://conversionrate-store.github.io/a-b_images/houseofworktops/customer_photos", S = {
  ...
  function x() {
    const s = document.querySelector("h1"), t = (s && s.textContent ? s.textContent : "").trim();
    return M(t || "Worktop");
  }
  function k() {
    ...
    if (!t) return null;
    const e = x();
    return Array.from({ length: t.count }, (n, o) => ({
      src: `${A}/${t.folder}/${o + 1}.webp`,
  ...
  class I {
    ...
    async init() {
      ...
      e.insertAdjacentElement("afterbegin", o), this.buildLightbox(), this.initSlider(o), this.wireCards(o), y(() => this.photos.forEach((i) => h(i.src))), this.$(o).on("pointerover", ".crs-cp__card", (i) => {
        const r = parseInt(
          i.currentTarget.getAttribute("data-index") || "-1",
          10
        );
        this.photos[r] && h(this.photos[r].src);
      }), P(
        o,
    ...
    buildSection() {
      const t = x(), e = this.photos.map(
        ...
        <div class="crs-cp__slide">
          <button class="crs-cp__card" type="button" data-index="${i}" aria-label="View ${o.alt}">
    ...
    buildLightbox() {
      ...
      }), this.stage.addEventListener("pointerdown", (i) => this.onPointerDown(i)), this.stage.addEventListener("pointermove", (i) => this.onPointerMove(i)), this.stage.addEventListener("pointerup", (i) => this.onPointerUp(i)), this.stage.addEventListener("pointercancel", () => this.dragging = !1), t(document).on("keydown.crsCp", (i) => {
    ...
    trapFocus(t, e) {
      ...
      if (!n.length) return;
      const o = n[0], i = n[n.length - 1], r = document.activeElement;
      e.shiftKey && r === o ? (e.preventDefault(), i.focus()) : !e.shiftKey && r === i && (e.preventDefault(), o.focus());
    }
    // ---- Zoom / pan (mobile) ----
    isMobile() {
    ...
    applyPan() {
      ...
      let i, r;
      if (n && o) {
        const a = Math.min(t / n, e / o);
        i = Math.max(0, (n * a * m - t) / 2), r = Math.max(0, (o * a * m - e) / 2);
      } else
        i = (m - 1) * t / 2, r = (m - 1) * e / 2;
      this.panX = Math.min(i, Math.max(-i, this.panX)), this.panY = Math.min(r, Math.max(-r, this.panY)), this.img.style.transform = `translate(${this.panX}px, ${this.panY}px) scale(${m})`;
    }
    onPointerDown(t) {
    ...
    onPointerMove(t) {
      ...
      (Math.abs(e) > 6 || Math.abs(n) > 6) && (this.moved = !0), this.zoomed && (this.panX = this.startPanX + e, this.panY = this.startPanY + n, this.applyPan(), t.preventDefault());
    }
    onPointerUp(t) {
      if (!this.dragging) return;
      this.dragging = !1;
      ...
      Math.abs(e) > 45 ? (this.showAt(this.modalIndex + (e < 0 ? 1 : -1)), c(
    ...
    setImage(t, e) {
      const n = ++this.loadToken, o = !!this.img.naturalWidth;
      this.img.alt = e, this.img.src = t, o || this.lb.classList.add("is-loading");
      const i = () => {
    ...
    closeLightbox() {
      var e;
      const t = this.lb;
      t && (this.zoomed && this.setZoom(!1), this.loadToken++, t.classList.remove("is-open", "is-loading"), t.setAttribute("aria-hidden", "true"), document.body.classList.remove("crs-cp-modal-open"), (e = this.lastFocused) == null || e.focus(), this.lastFocused = null);
    }
  }
  const H = `/* Product gallery viewer — our reels-style lightbox that replaces the host's
...
`, p = "Product gallery", w = ".image-popup a[href]", C = ".worktop-thumbnail-slider img", V = "https://houseofworktops.co.uk/catalog/view/theme/houseofworktops/image/svg-icons/video-previous.svg", q = "https://houseofworktops.co.uk/catalog/view/theme/houseofworktops/image/svg-icons/video-next.svg", b = 2;
  class F {
    ...
    async init() {
      ...
      const t = k();
      !t || !t.length || (this.customerPhotos = t, await _(w), g(H), this.buildViewer(), document.addEventListener("click", (e) => this.onGalleryClick(e), !0), y(() => {
    ...
    onIntent(t) {
      ...
      const n = e.closest(w), o = n || e.closest(C);
      o && h((n ? n.getAttribute("href") : o.getAttribute("src")) || "");
    }
    // --- Product photos: read the ordered, de-duplicated full-res URLs live ---
    getProductPhotos() {
      const t = Array.from(
        document.querySelectorAll(w)
      ), e = /* @__PURE__ */ new Set(), n = [], o = x();
      return t.forEach((i) => {
        ...
        if (e.has(a)) return;
        e.add(a);
        const l = i.querySelector("img"), U = l && l.getAttribute("alt") || `${o} product photo`;
        n.push({ src: r, alt: U });
      }), n;
    }
    ...
    onGalleryClick(t) {
      ...
      if (!i.length) return;
      const r = this.stem(
        (n ? n.getAttribute("href") : o.getAttribute("src")) || ""
      ), a = i.findIndex((l) => this.stem(l.src) === r);
    ...
    openCombined(t, e) {
      ...
      const n = this.lb.querySelector('.crs-pg-lb__seg[data-group="product"]'), o = this.lb.querySelector('.crs-pg-lb__seg[data-group="kitchen"]');
      n && (n.textContent = `Product photos (${t.length})`), o && (o.textContent = `Real kitchens (${this.customerPhotos.length})`), this.open(e);
    }
    ...
    buildViewer() {
      ...
      <button class="crs-pg-lb__nav crs-pg-lb__nav--prev" type="button" aria-label="Previous photo">
        <img src="${V}" alt="" width="23" height="46" aria-hidden="true" />
      </button>
    ...
    applyPan() {
      ...
      if (n && o) {
        const a = Math.min(t / n, e / o);
        i = Math.max(0, (n * a * b - t) / 2), r = Math.max(0, (o * a * b - e) / 2);
      } else
        i = (b - 1) * t / 2, r = (b - 1) * e / 2;
      this.panX = Math.min(i, Math.max(-i, this.panX)), this.panY = Math.min(r, Math.max(-r, this.panY)), this.img.style.transform = `translate(${this.panX}px, ${this.panY}px) scale(${b})`;
    }
    onPointerDown(t) {
      this.dragging = !0, this.moved = !1, this.sx = t.clientX, this.sy = t.clientY, this.startPanX = this.panX, this.startPanY = this.panY;
      try {
    ...
    onPointerUp(t) {
      ...
      Math.abs(e) > 45 ? (this.showAt(this.index + (e < 0 ? 1 : -1)), c(
    ...
    showAt(t, e = !1) {
      ...
      i.group === "kitchen" && !this.kitchenViewed && (this.kitchenViewed = !0, c("exp_gallery_kitchen_view", "First real kitchen photo", "view", p)), this.setImage(i.src, i.alt), h(this.slides[(o + 1) % n].src), h(this.slides[(o - 1 + n) % n].src), this.lb.querySelectorAll(".crs-pg-lb__seg").forEach((r) => {
        r.classList.toggle(
          "is-active",
    ...
    trapFocus(t) {
      ...
      if (!e.length) return;
      const n = e[0], o = e[e.length - 1], i = document.activeElement;
      t.shiftKey && i === n ? (t.preventDefault(), o.focus()) : !t.shiftKey && i === o && (t.preventDefault(), n.focus());
    }
  }
  const X = `/* Outer spacing for the relocated blocks, matching Figma.
...
`;
  class j {
    ...
    async init() {
      ...
      for (g(X); t.firstChild && t.firstChild.nodeType === Node.TEXT_NODE; )
  ...
  const O = `/* Trustpilot rating row (Figma node 2433:1324): centered "Excellent 4.8 ★★★★★ Trustpilot"
...
`, Z = `<svg preserveAspectRatio="none" width="100%" height="100%" overflow="visible" style="display: block;" viewBox="0 0 77 19" fill="none" xmlns="http://www.w3.org/2000/svg">
<g id="Logo &#226;&#134;&#146; Img">
...
`, D = "https://widget.trustpilot.com/trustbox-data/5419b6ffb0d04a076446a9af?businessUnitId=5de985d57cad6c0001a1286e&locale=en-GB", R = "https://cdn.trustpilot.net/brand-assets/4.1.0/stars", f = {
    trustScore: 4.8,
    stars: 5,
  ...
  class Y {
    ...
    async fetchData() {
      try {
        ...
        return u("TrustpilotRating: live data fetch failed, using fallback", "warn"), { ...f };
      }
    }
    ...
    fillRow(t, e) {
      ...
      <img
        class="crs-tp__stars"
        src="${R}/stars-${this.starsFile(e.stars)}.svg"
        alt=""
        width="100"
      />
      <span class="crs-tp__logo" aria-hidden="true">${Z}</span>`;
    }
  }
  const N = "";
  L({ name: "PDP Introducing Customer Photos", dev: "OS" }), z("exp_pdp_photo");
  class B {
    constructor() {
      this.init();
    }
    init() {
      g(N), new I(), new F(), new j(), new Y();
    }
  }
  new B();
})();
