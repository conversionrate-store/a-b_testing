...
`,C=`Product gallery`,w=`.image-popup`,T=`.worktop-thumbnail-slider`,E=`crs-kitchen`,D=4,O=7,k=class{constructor(){this.slider=null,this.segs=[],this.productCount=0,this.kitchenFirst=!1,this.expanded=!1,this.init()}async init(){if(document.querySelector(`.crs-gsw`))return;let e=g();if(!e||!e.length)return;let n=(await t(w)).parentElement;n&&(this.row=n,this.productCount=n.querySelectorAll(w).length,this.slider=document.querySelector(T),o(S),this.addTiles(e),this.addSlides(e),this.mount(e.length),this.applyOrder(),this.moveThumb(!0),document.getElementById(`loadMore`)?.addEventListener(`click`,()=>{this.expanded=!0,this.applyVisibility()}),window.addEventListener(`resize`,()=>{this.moveThumb(!0),this.capDots()}))}addTiles(e){let t=e.map(e=>`
      ...
      </div>`;r?n(t).slick(`slickAdd`,i):t.insertAdjacentHTML(`beforeend`,i)}),n&&(n(t).on(`reInit`,()=>this.syncSlider()),this.followSlider(n,t),n(t).on(`beforeChange`,(e,t,n,r)=>this.capDots(r)),n(t).on(`init reInit setPosition afterChange`,()=>this.capDots()),this.capDots())}followSlider(e,t){let n=0,r=()=>n=Date.now();e(t).on(`swipe`,r),document.addEventListener(`click`,e=>{e.target?.closest?.(`.thumbnail-arrows`)&&r()},!0),e(t).on(`afterChange`,(e,t,r)=>{Date.now()-n>1200||(n=0,this.setGroup(r>=this.productCount,!1))})}mount(t){let n=document.createElement(`div`);n.className=`crs-gsw`,n.innerHTML=`
...
.crs-pg-lb__frame {
  position: relative;
  flex-shrink: 0;
  width: var(--crs-pg-size);
  height: var(--crs-pg-size);
}
.crs-pg-lb__stage {
  position: relative;
  z-index: 1;
...
.crs-pg-lb:not(.is-zoomed) .crs-pg-lb__img {
  cursor: zoom-in;
}
...
.crs-pg-lb.is-zoomed.is-grabbing .crs-pg-lb__stage,
.crs-pg-lb.is-zoomed.is-grabbing .crs-pg-lb__img {
  cursor: grabbing;
}
...
@media (max-width: 768px) {
  .crs-pg-lb {
    --crs-pg-side: max(180px, min(100vw, calc(100vh - 200px)));
    --crs-pg-side: max(180px, min(100vw, calc(100dvh - 200px)));
  ...
  .crs-pg-lb__switch {
    position: fixed;
    top: calc(var(--crs-pg-margin) + 49px);
    gap: 8px;
    padding: 6px;
    background: rgba(9, 59, 50, 0.8);
  }
  .crs-pg-lb__seg {
    min-height: 38px;
    padding: 12px;
  ...
  .crs-pg-lb__nav {
    position: fixed;
    top: calc(var(--crs-pg-top) + var(--crs-pg-side) / 2);
    transform: translateY(-50%);
    padding: 8px;
    z-index: 5;
  }
  .crs-pg-lb__nav--prev {
    left: calc(50vw - var(--crs-pg-side) / 2 + 8px);
  }
  .crs-pg-lb__nav--next {
    right: calc(50vw - var(--crs-pg-side) / 2 + 8px);
  }
  .crs-pg-lb__nav img {
    width: 14px;
    height: 28px;
...
`,j=`Product gallery`,M=`.image-popup a[href]`,N=`.worktop-thumbnail-slider img`,P=`.crs-gsw-tile img`,F=`https://houseofworktops.co.uk/catalog/view/theme/houseofworktops/image/svg-icons/video-previous.svg`,I=`https://houseofworktops.co.uk/catalog/view/theme/houseofworktops/image/svg-icons/video-next.svg`,L=2,R=20,z=class{constructor(){this.customerPhotos=[],this.slides=[],this.boundary=0,this.index=0,this.loadToken=0,this.kitchenViewed=!1,this.ghostDir=0,this.lastFocused=null,this.zoomed=!1,this.dragging=!1,this.moved=!1,this.downOnImg=!1,this.swiping=!1,this.sx=0,this.sy=0,this.startPanX=0,this.startPanY=0,this.panX=0,this.panY=0,this.init()}async init(){if(document.getElementById(`crsPgLightbox`))return;let e=g();!e||!e.length||(this.customerPhotos=e,await t(M),o(A),this.buildViewer(),document.addEventListener(`click`,e=>this.onGalleryClick(e),!0),u(()=>{this.customerPhotos.forEach(e=>l(e.src)),this.getProductPhotos().forEach(e=>l(e.src))}),document.addEventListener(`pointerover`,e=>this.onIntent(e),!0))}onIntent(e){let t=e.target;if(!t||!t.closest)return;let n=t.closest(M),r=n||t.closest(N);r&&l((n?n.getAttribute(`href`):r.getAttribute(`src`))||``)}getProductPhotos(){let e=Array.from(document.querySelectorAll(M)),t=new Set,n=[],r=h();return e.forEach(e=>{let i=e.getAttribute(`href`);if(!i)return;let a=this.stem(i);if(t.has(a))return;t.add(a);let o=e.querySelector(`img`),s=o&&o.getAttribute(`alt`)||`${r} product photo`;n.push({src:i,alt:s})}),n}stem(e){try{e=decodeURIComponent(e)}catch{}return(e.split(`?`)[0].split(`#`)[0].split(`/`).pop()||``).replace(/-\d+x\d+(?=\.\w+$)/,``).replace(/\.\w+$/,``).toLowerCase()}onGalleryClick(t){let n=t.target;if(!n||!n.closest)return;let r=n.closest(M),i=r||n.closest(N)||n.closest(P);if(!i)return;t.preventDefault(),t.stopImmediatePropagation();let a=this.getProductPhotos();if(!a.length)return;this.boundary=a.length,this.slides=[...a.map(e=>({...e,group:`product`})),...this.customerPhotos.map(e=>({...e,group:`kitchen`}))];let o=this.stem((r?r.getAttribute(`href`):i.getAttribute(`src`))||``),s=this.slides.findIndex(e=>this.stem(e.src)===o),c=this.lb.querySelector(`.crs-pg-lb__seg[data-group="product"]`),l=this.lb.querySelector(`.crs-pg-lb__seg[data-group="kitchen"]`);c&&(c.textContent=`Product images (${a.length})`),l&&(l.textContent=`Real kitchens (${this.customerPhotos.length})`);let u=s<0?0:s;e(`exp_gallery_open`,`Opened photo ${u+1}`,`click`,j),this.open(u)}buildViewer(){let t=document.createElement(`div`);t.className=`crs-pg-lb`,t.id=`crsPgLightbox`,t.setAttribute(`role`,`dialog`),t.setAttribute(`aria-modal`,`true`),t.setAttribute(`aria-hidden`,`true`),t.setAttribute(`aria-label`,`Product photo viewer`),t.innerHTML=`
      <p class="crs-pg-lb__title">${h()}</p>
      <div class="crs-pg-lb__row">
        <button class="crs-pg-lb__nav crs-pg-lb__nav--prev" type="button" aria-label="Previous photo">
          <img src="${F}" alt="" width="23" height="46" aria-hidden="true" />
        </button>
        <div class="crs-pg-lb__frame">
          ...
          <div class="crs-pg-lb__stage">
            ...
            <button class="crs-pg-lb__zoom" type="button">Tap image to zoom</button>
          </div>
        </div>
        <button class="crs-pg-lb__nav crs-pg-lb__nav--next" type="button" aria-label="Next photo">
          <img src="${I}" alt="" width="23" height="46" aria-hidden="true" />
        </button>
      </div>
      <div class="crs-pg-lb__spinner" aria-hidden="true"></div>`,document.body.appendChild(t),this.lb=t,this.img=t.querySelector(`#crsPgImg`),this.ghost=t.querySelector(`.crs-pg-lb__img--ghost`),this.stage=t.querySelector(`.crs-pg-lb__stage`),this.thumb=t.querySelector(`.crs-pg-lb__thumb`),t.querySelector(`.crs-pg-lb__close`).addEventListener(`click`,()=>{e(`exp_gallery_close`,`Close`,`click`,j),this.close()}),t.querySelector(`.crs-pg-lb__nav--prev`).addEventListener(`click`,()=>this.navigate(-1,`Arrow Left`)),t.querySelector(`.crs-pg-lb__nav--next`).addEventListener(`click`,()=>this.navigate(1,`Arrow Right`)),t.querySelectorAll(`.crs-pg-lb__seg`).forEach(t=>{t.addEventListener(`click`,()=>{let n=t.getAttribute(`data-group`),r=n===`product`?`Product images`:`Real kitchens`;this.showAt(n===`kitchen`?this.boundary:0),e(`exp_gallery_full_switch`,r,`click`,j)})});let n=t.querySelector(`.crs-pg-lb__zoom`);n.addEventListener(`pointerdown`,e=>e.stopPropagation()),n.addEventListener(`click`,e=>{e.stopPropagation(),this.setZoom(!0)}),t.addEventListener(`click`,n=>{n.target===t&&(e(`exp_gallery_overlay`,`Overlay close`,`click`,j),this.close())}),this.stage.addEventListener(`pointerdown`,e=>this.onPointerDown(e)),this.stage.addEventListener(`pointermove`,e=>this.onPointerMove(e)),this.stage.addEventListener(`pointerup`,e=>this.onPointerUp(e)),this.stage.addEventListener(`pointercancel`,()=>{this.dragging=!1,this.lb.classList.remove(`is-grabbing`),this.zoomed||this.springBack()}),document.addEventListener(`keydown`,e=>{t.getAttribute(`aria-hidden`)!==`true`&&(e.key===`Escape`?this.close():e.key===`ArrowLeft`?this.showAt(this.index-1):e.key===`ArrowRight`?this.showAt(this.index+1):e.key===`Tab`&&this.trapFocus(e))})}isMobile(){return window.matchMedia(`(max-width: 480px)`).matches}setZoom(t){this.zoomed=t,this.panX=0,this.panY=0,this.lb.classList.toggle(`is-zoomed`,t),this.img.style.transition=`transform 0.25s ease`,t?(requestAnimationFrame(()=>this.applyPan()),e(`exp_gallery_zoom`,`Zoom ${this.index+1}`,`click`,j)):this.img.style.transform=``}applyPan(){let e=this.stage.clientWidth,t=this.stage.clientHeight,n=this.img.naturalWidth,r=this.img.naturalHeight,i,a;if(n&&r){let o=Math.min(e/n,t/r);i=Math.max(0,(n*o*L-e)/2),a=Math.max(0,(r*o*L-t)/2)}else i=(L-1)*e/2,a=(L-1)*t/2;this.panX=Math.min(i,Math.max(-i,this.panX)),this.panY=Math.min(a,Math.max(-a,this.panY)),this.img.style.transform=`translate(${this.panX}px, ${this.panY}px) scale(${L})`}onPointerDown(e){if(!this.swiping){this.dragging=!0,this.moved=!1,this.downOnImg=e.target===this.img,this.img.style.transition=``,this.lb.classList.add(`is-grabbing`),this.sx=e.clientX,this.sy=e.clientY,this.startPanX=this.panX,this.startPanY=this.panY;try{this.stage.setPointerCapture(e.pointerId)}catch{}}}onPointerMove(e){if(!this.dragging)return;let t=e.clientX-this.sx,n=e.clientY-this.sy;if((Math.abs(t)>6||Math.abs(n)>6)&&(this.moved=!0),this.zoomed)this.panX=this.startPanX+t,this.panY=this.startPanY+n,this.applyPan(),e.preventDefault();else if(this.moved&&this.isMobile()){let n=this.swipeW()+R,r=Math.max(-n,Math.min(n,t)),i=t<0?1:-1;this.setGhost(i),this.img.style.transform=`translateX(${r}px)`,this.ghost.style.transform=`translateX(${r+i*n}px)`,e.preventDefault()}}onPointerUp(t){if(this.lb.classList.remove(`is-grabbing`),!this.dragging)return;this.dragging=!1;let n=t.clientX-this.sx;if(this.zoomed){this.moved||this.setZoom(!1);return}if(Math.abs(n)>this.swipeW()/5){let t=n<0?1:-1,r=()=>e(`exp_gallery_full_swipe`,`Swipe ${t>0?`left`:`right`} - ${this.index+1}`,`other`,j);this.isMobile()?this.animateSwipe(t,r):(this.showAt(this.index+t),r())}else this.moved?this.springBack():this.downOnImg&&this.setZoom(!0)}swipeW(){return this.stage.clientWidth||window.innerWidth}setGhost(e){let t=this.slides.length,n=((this.index+e)%t+t)%t,r=this.slides[n].src;this.ghost.getAttribute(`src`)!==r&&(this.ghost.src=r),this.ghost.style.visibility=`visible`,this.ghostDir=e}hideGhost(){this.ghost.style.visibility=`hidden`,this.ghost.style.transition=``,this.ghostDir=0}navigate(t,n){if(this.swiping)return;let r=()=>e(`exp_gallery_nav`,`${n} - ${this.index+1}`,`click`,j);this.isMobile()&&!this.zoomed?this.animateSwipe(t,r):(this.showAt(this.index+t),r())}animateSwipe(e,t){this.swiping=!0;let n=this.img,r=this.ghost,i=this.swipeW()+R;this.ghostDir!==e&&(this.setGhost(e),r.style.transition=`none`,r.style.transform=`translateX(${e*i}px)`),n.style.transition=r.style.transition=`transform 0.26s ease-out`,n.style.transform=`translateX(${-e*i}px)`,r.style.transform=`translateX(0px)`,setTimeout(()=>{this.showAt(this.index+e),t(),n.style.transition=`none`;let r=()=>{n.style.transform=``,this.hideGhost(),requestAnimationFrame(()=>{n.style.transition=``,this.swiping=!1})};n.decode?n.decode().then(r,r):r()},270)}springBack(){let e=this.img,t=this.ghost;e.style.transition=`transform 0.18s ease`,e.style.transform=``,this.ghostDir&&(t.style.transition=`transform 0.18s ease`,t.style.transform=`translateX(${this.ghostDir*(this.swipeW()+R)}px)`),setTimeout(()=>{e.style.transition=``,this.hideGhost()},200)}showAt(t,n=!1){let r=this.slides.length,i=(t%r+r)%r;this.index=i,this.zoomed&&this.setZoom(!1);let a=this.slides[i];a.group===`kitchen`&&!this.kitchenViewed&&(this.kitchenViewed=!0,e(`exp_gallery_kitchen_view`,`First real kitchen photo`,`view`,j)),this.setImage(a.src,a.alt),l(this.slides[(i+1)%r].src),l(this.slides[(i-1+r)%r].src),this.lb.querySelectorAll(`.crs-pg-lb__seg`).forEach(e=>{e.classList.toggle(`is-active`,e.getAttribute(`data-group`)===a.group)}),this.moveThumb(n)}setImage(e,t){let n=++this.loadToken,r=!!this.img.naturalWidth;this.img.alt=t,this.img.src=e,r||this.lb.classList.add(`is-loading`);let i=()=>{n===this.loadToken&&this.lb.classList.remove(`is-loading`)};this.img.decode?this.img.decode().then(i,i):i()}moveThumb(e=!1){let t=this.lb.querySelector(`.crs-pg-lb__seg.is-active`);t&&(e&&(this.thumb.style.transition=`none`),this.thumb.style.width=`${t.offsetWidth}px`,this.thumb.style.height=`${t.offsetHeight}px`,this.thumb.style.transform=`translate(${t.offsetLeft}px, ${t.offsetTop}px)`,e&&(this.thumb.offsetWidth,requestAnimationFrame(()=>this.thumb.style.transition=``)))}open(e){this.lastFocused=document.activeElement,this.showAt(e,!0),this.lb.classList.add(`is-open`),this.lb.setAttribute(`aria-hidden`,`false`),document.body.classList.add(`crs-pg-modal-open`),this.lb.querySelector(`.crs-pg-lb__close`)?.focus()}close(){this.zoomed&&this.setZoom(!1),this.kitchenViewed=!1,this.swiping=!1,this.img.style.transition=``,this.img.style.transform=``,this.ghost.style.transform=``,this.hideGhost(),this.loadToken++,this.lb.classList.remove(`is-open`,`is-loading`),this.lb.setAttribute(`aria-hidden`,`true`),document.body.classList.remove(`crs-pg-modal-open`),this.lastFocused?.focus(),this.lastFocused=null}trapFocus(e){let t=Array.from(this.lb.querySelectorAll(`button`)).filter(e=>e.offsetParent!==null);if(!t.length)return;let n=t[0],r=t[t.length-1],i=document.activeElement;e.shiftKey&&i===n?(e.preventDefault(),r.focus()):!e.shiftKey&&i===r&&(e.preventDefault(),n.focus())}},B=`.mb-4:has(> #delivery_check) {
  margin-top: 1.5rem;
}
...
`,U=`<svg preserveAspectRatio="none" width="100%" height="100%" overflow="visible" style="display: block;" viewBox="0 0 77 19" fill="none" xmlns="http://www.w3.org/2000/svg">
<g id="Logo &#226;&#134;&#146; Img">
...
`,W=`https://widget.trustpilot.com/trustbox-data/5419b6ffb0d04a076446a9af?businessUnitId=5de985d57cad6c0001a1286e&locale=en-GB`,G=`https://cdn.trustpilot.net/brand-assets/4.1.0/stars`,K={trustScore:4.8,stars:5,profileUrl:`https://uk.trustpilot.com/review/houseofworktops.co.uk`},q=class{constructor(){this.init()}async init(){if(document.querySelector(`.crs-tp`))return;let e=(await t(`#reels-slider-section .reels-heading`)).parentElement;if(!e)return;o(H),this.hideOriginal();let n=this.buildRow(K);e.insertAdjacentElement(`afterbegin`,n),this.fetchData().then(e=>this.fillRow(n,e))}hideOriginal(){o(`#product .trustpilot-widget{display:none !important;}#product hr.mt-4:has(+ .trustpilot-widget){display:none !important;}`)}async fetchData(){try{let e=await fetch(W);if(!e.ok)throw Error(`HTTP ${e.status}`);let t=await e.json(),n=t.businessUnit||t.businessEntity||{};return{trustScore:typeof n.trustScore==`number`?n.trustScore:K.trustScore,stars:typeof n.stars==`number`?n.stars:K.stars,profileUrl:t.links&&t.links.profileUrl||K.profileUrl}}catch{return{...K}}}label(e){return e>=4.3?`Excellent`:e>=3.8?`Great`:e>=3?`Average`:e>=2?`Poor`:`Bad`}starsFile(e){let t=Math.round(e*2)/2;return t%1==0?String(t):t.toFixed(1)}buildRow(e){let t=document.createElement(`a`);return t.className=`crs-tp`,t.target=`_blank`,t.rel=`noopener noreferrer`,this.fillRow(t,e),t}fillRow(e,t){let n=this.label(t.trustScore),r=t.trustScore.toFixed(1);e.href=t.profileUrl,e.setAttribute(`aria-label`,`${n} ${r} out of 5 on Trustpilot`),e.innerHTML=` <span class="crs-tp__rating">
        <span class="crs-tp__word">${n}</span>
        <span class="crs-tp__score">${r}</span>
      </span>
      <img
        class="crs-tp__stars"
        src="${G}/stars-${this.starsFile(t.stars)}.svg"
        alt=""
        width="100"
      />
      <span class="crs-tp__logo" aria-hidden="true">${U}</span>`}};n({name:`PDP Introducing Customer Photos v4`,dev:`OS`}),r(`exp_pdp_photo`),new class{constructor(){this.init()}init(){o(`
`),new x,new k,new z,new V,new q}}})();
